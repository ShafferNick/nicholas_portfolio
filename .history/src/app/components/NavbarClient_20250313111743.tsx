    ...
    <nav className="bg-transparent text-white p-4 sticky top-0 z-10">
      <div className="max-w-6xl mx-auto flex items-center justify-between px-6">
        ...
        <div className="md:hidden relative">
          ...
          {/* Mobile Menu */}
          <div
            className={`md:hidden fixed top-0 right-0 w-64 max-h-screen bg-gray-200 p-6 pt-32 box-border overflow-y-auto transform transition-transform duration-500 ease-[cubic-bezier(0.77,0.2,0.05,1.0)] z-10 ${
              isOpen ? 'translate-x-0' : 'translate-x-full'
            }`}
          >
            <div className="flex flex-col space-y-4">
              <Link href="/" className="text-gray-800 hover:text-red-400 transition-colors duration-300 ease-in-out text-xl py-2 text-center" onClick={() => setIsOpen(false)}>Home</Link>
              <Link href="/about" className="text-gray-800 hover:text-red-400 transition-colors duration-300 ease-in-out text-xl py-2 text-center" onClick={() => setIsOpen(false)}>About Me</Link>
              <Link href="/contact" className="text-gray-800 hover:text-red-400 transition-colors duration-300 ease-in-out text-xl py-2 text-center" onClick={() => setIsOpen(false)}>Contact</Link>
              <Link href="/projects" className="text-gray-800 hover:text-red-400 transition-colors duration-300 ease-in-out text-xl py-2 text-center" onClick={() => setIsOpen(false)}>Projects</Link>
            </div>
          </div>
        </div>
      </div>
